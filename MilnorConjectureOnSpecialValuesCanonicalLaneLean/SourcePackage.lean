/-!
# Source package model for `milnor-conjecture-on-special-values-canonical-lane`
...
def sourceCheckoutHead : String := "137682750bc9dbc75964365e9ed4ed15d1237f8d"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/msv_closure_guard.py", sha256 := "239cfbb2f4d0bfbb528eb6aa753bc842ffe01986e6e2fe50077e67eec2b9a50f", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/msv_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/msv_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/msv_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/msv_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/msv_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/msv_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/msv_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/msv_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/msv_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_polylog\", \"sigma_value\", \"kappa_compact\", \"rho_rigidity\", \"regulator_transfer\"]", line := 21 },
  { file := "scripts/msv_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_polylog", status := "derived_numeric", formula := "c_polylog_raw * polylog_density_raw - e_polylog_raw", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_polylog_raw", "e_polylog_raw", "polylog_density_raw"] },
  { group := "constants", key := "sigma_value", status := "derived_numeric", formula := "value_floor_raw - regulator_loss_raw - restart_loss_raw", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["regulator_loss_raw", "restart_loss_raw", "value_floor_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "regulator_transfer", status := "derived_numeric", formula := "c_reg_raw * transfer_gain_raw - e_reg_raw", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_reg_raw", "e_reg_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Appendix E.6" },
  { key := "kappa_compact", value := "0.8045052292839904", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "kappa_polylog", value := "1.0932", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_polylog", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "regulator_transfer", value := "1.029422", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#regulator_transfer", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "rho_rigidity", value := "1.078", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_value", value := "1.0750000000000002", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_value", sourceSection := "paper/MILNOR_CONJECTURE_ON_SPECIAL_VALUES_PREPRINT.md Section 5.2 / Appendix B" }
]
